...
const newuser = () => {
    ...
    const { id, name, gender, email, password, phoneNumber, jobTitle } = useLocalSearchParams()
    const token = useSelector((state) => state.user.token)
    const [isSubmitting, setisSubmitted] = useState(false)
    const [isMale, setisMale] = useState(gender === 'male' ? true : false)
    const [isFemale, setisFemale] = useState(gender === 'female' ? true : false)
    ...
    const submit = async () => {
        try {
            ...
            const response = await axiosPrivate.put("/users/update-user",
                form,
                {
                    headers: {
            ...
            if (response.status === 200) {
                Alert.alert('Success!!', 'User Update!')
                setform({
                    name: '',
                    email: '',
                    phoneNumber: '',
