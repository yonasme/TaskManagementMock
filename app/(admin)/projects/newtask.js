import { View, Text, ScrollView, Image, Alert, Pressable, TouchableOpacity, TextInput, Button, FlatList } from 'react-native'
...
const newtask = () => {
    ...
    const submit = async () => {
        try {
            ...
            const response = await axiosPrivate.post("/tasks/create-task",
                form,
                {
                    headers: {
    ...
    // const debouncedOnChange = debounce((item) => {
    //     // setFleetLicensePlateUpdate(item.label);
    //     // setFleetVehiclesID(item.value);
    //     // fetchFleetVehicleData(item.value);
    // }, 300);

    const renderItem = ({ item }) => (
        <View className=" ml-2 mt-2 bg-black-100 rounded-md w-full flex-1">
            <View className="flex-row h-16">
                <Text className="items-center text-lg text-gray-100 font-pmedium ">{item}</Text>
                <TouchableOpacity className=" ml-1" onPress={() => handleRemoveTag(item)}             >
                    <Image
